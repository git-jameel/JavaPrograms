...
public class LibMan {
	...
	public static class Admin extends AdminParent {
		...
		public static void add() {
			...
			name = getDefaultBookNames(name, newName);
			price = getDefaultBookPrices(price, newPrice);
			stock = getDefaultBookPrices(stock, newStock);
			System.out.println("BookId" + "\tname" + "\tPrice" + "\tStock");
			for (int i = 1; i <= name.length; i++) {
				System.out.print(i + "\t" + name[i - 1] + "\t" + price[i - 1] + "\t" + stock[i - 1] + "\n");
			}
	...
	public static class AdminParent {
		public static String[] getDefaultBookNames(String[] name, String[] newName) {
		...
		public static int[] getDefaultBookPrices(int[] price, int[] newPrice) {
		...
		public static int[] getDefaultBookStocks(int[] stock, int[] newStock) {
			...
			for (int i = 0; i < stockUpdated.length; i++) {
				if (i < stock.length) {
					...
					stockUpdated[i] = newStock[i - stock.length - 1];
				}
			}
			return stockUpdated;
		}
